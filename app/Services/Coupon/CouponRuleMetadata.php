    ...
    public static function for(EligibilityRuleType $rule): array
    {
        return match ($rule) {
            ...
            EligibilityRuleType::AREA_IN => [
                ...self::base($rule,
                    labelEn: 'Saved address area in list',
                    labelAr: 'منطقة العنوان المحفوظ ضمن القائمة',
                    descEn: 'Requires the customer to have at least one saved address (address.governorate_id, active governorates only) in the allowed list. Accepts a single id or a non-empty array of ids; strict positive integers only. Evaluated strictly at every stage from the authenticated user\'s own addresses; delivery/shipping governorate is irrelevant. Addresses without a governorate never match.',
                    descAr: 'يشترط أن يكون لدى العميل عنوان محفوظ واحد على الأقل (address.governorate_id للمحافظات النشطة فقط) ضمن القائمة المسموحة. يقبل معرفاً واحداً أو مصفوفة غير فارغة من المعرفات الصحيحة الموجبة فقط. يتم التقييم بصرامة في كل مرحلة من عناوين المستخدم الخاصة؛ محافظة التوصيل/الشحن غير ذات صلة. العناوين بلا محافظة لا تطابق أبداً.',
