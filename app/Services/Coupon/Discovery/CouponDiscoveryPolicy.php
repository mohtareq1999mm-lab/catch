 ...
 * - assignment-only: assignments exist, no targeting, NOT publicly
 *                   discoverable (personal grants — private to assignees via
 *                   My Coupons, never the catalog).
 * - public:         publicly discoverable (explicit is_public flag) OR the
 *                   legacy no-configuration state (no assignments, no
 *                   targeting). Assignments NEVER demote a public coupon.
 *
...
class CouponDiscoveryPolicy
{
    ...
    public function decide(Coupon $coupon, ?User $user, ?CouponClaim $claim = null): array
    {
        ...
        if ($targeting !== null) {
            ...
            // Public discoverability is independent of assignments: an
            // assignment never demotes a public coupon. Only private
            // (flag off) assigned coupons are assignment-only.
            $isPublic = (bool) ($coupon->getAttribute('is_public') ?? false);
            $visibility = (! $hasAssignments || $isPublic) ? 'public' : 'assignment-only';
        }
